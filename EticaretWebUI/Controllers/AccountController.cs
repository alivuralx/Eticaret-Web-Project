...
namespace EticaretWebUI.Controllers
{
    public class AccountController : Controller
    {
                ...
                try
                {
                    ...
                    if (user is not null)
                    {
                        ...
                        if (sonuc > 0)
                        {
                            ...
                            <strong>Bilgileriniz Güncellenmiştir!</strong><button type=""button"" class=""btn-close"" data-bs-dismiss=""alert"" aria-label=""Close""></button>
                             </div>";
        ...
        public async Task<IActionResult> PasswordRenewAsync(string Email)
        {
            ...
            string mesaj = $"Sayın {user.Name} {user.Surname} <br> Şifrenizi yenilemek için lütfen  : <a href='http://localhost:5082/Account/PasswordChange?user={user.UserGuid.ToString()}'>linke tıklayınız.</a>";
            var sonuc = await MailHelper.SendMailAsync(Email, "Şifremi Yenile", mesaj);
            if (sonuc)
            {
                TempData["Message"] = @"<div class=""alert alert-success alert-dismissible fade show"" role=""alert"">
                            <strong>Şifre Sıfırlama Bağlantınız Mail Adresinize gönderilmiştir!</strong><button type=""button"" class=""btn-close"" data-bs-dismiss=""alert"" aria-label=""Close""></button>
                             </div>";
            }
            else
            {
                TempData["Message"] = @"<div class=""alert alert-danger alert-dismissible fade show"" role=""alert"">
                            <strong>Şifre Sıfırlama Bağlantınız Mail Adresinize Gönderilemedi!!</strong><button type=""button"" class=""btn-close"" data-bs-dismiss=""alert"" aria-label=""Close""></button>
                             </div>";
            }
            return View();
        }
        ...
        public async Task<IActionResult> PasswordChange(string user, string Password)
        {
            ...
            {
                TempData["Message"] = @"<div class=""alert alert-success alert-dismissible fade show"" role=""alert"">
                            <strong>Şifreniz Güncellenmiştir! Giriş Ekranından Oturum AÇabilirsiniz</strong><button type=""button"" class=""btn-close"" data-bs-dismiss=""alert"" aria-label=""Close""></button>
                             </div>";
            }
            else
            {
